...
import VideoShowEdit from './VideoShowEdit';
import TopBar from './TopBar';

import {Grid, Paper, Typography, Box, Button, TextField, AppBar, Tabs, Tab} from '@material-ui/core';
...
export default class MarkInfo extends React.Component {
  constructor(props){
    super(props);
    const nMarks = "Marca " + (this.props.marks.length+1).toString();
    this.state = {
        title: nMarks,
        time: 0,
        timeHMS: [0,0,0],
    ...
    handleSubmit = (event) => {
      const nMarksCurrent = this.props.marks.length;
      ...
      if (this.state.title.length !== 0 && this.state.time.length !== 0) {
        if (this.state.type === 'TEST' || this.state.type === 'QUESTION') {
          if (this.state.content.question.length !== 0) {
            this.props.dispatch(addMark(this.state));
            this.loadInitialState(nMarksCurrent);
          } else {
            return null;
          }
    ...
    return(
      ...
      <Grid container >
        ...
        <Grid item xs={12} lg={6} style={{padding: '20px'}}>
          <Grid container direction="column" style={{paddingBottom: '15px'}}>
            <Grid item style={{paddingBottom: '25px'}}>
              <Typography variant="h5">Añadir marca</Typography>
            </Grid>
            <Grid item style={{paddingBottom: '25px'}}>
        ...
        <Grid item xs={12} lg={6} style={{padding: '20px'}}>
          ...
          <Paper>
            <VideoShowEdit url={this.props.url} marks={this.props.marks} title={this.props.title} description={this.props.description} currentSecond={this.state.time} handleTimeVideo={this.handleTimeVideo} currentTime={this.state.timeHMS}/>
          </Paper>


            <MarkList dispatch={this.props.dispatch} marks={this.props.marks} />
