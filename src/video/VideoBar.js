...
import {Slider, Popover, Grid, Paper, Box, Button, Typography} from '@material-ui/core';

import SoundController from './SoundController';

import { PlayArrow, Pause, LocationOnIcon } from '@material-ui/icons';
...
export default class VideoBar extends React.Component {
  ...
  render() {
    ...
    let time = '00:00';
    const hoursText = this.props.currentTime[0].toString().padStart(2,'0');
    const minutesText = this.props.currentTime[1].toString().padStart(2,'0');
    const secondsText = this.props.currentTime[2].toString().padStart(2,'0');
    if (this.props.currentTime[0] === 0) {
      ...
      if (this.props.insideMark === false) {
          ...
          <Box display="flex" style={{backgroundColor: '#ECECEC', height: '40px'}}>
            ...
            <Box p={1} flexGrow={1}>
              <div style={{position: 'relative'}}>
                <Slider style={{ width: '100%'}} min={0} max={this.props.duration} value={this.props.currentSecond} onChange={this.handleRange} valueLabelDisplay="auto" valueLabelFormat={() => time} />
                {
        ...
        return (
          <Box display="flex" style={{backgroundColor: '#ECECEC', height: '40px'}}>
            ...
            <Box p={1} flexGrow={1}>
              <div style={{position: 'relative'}}>
                <Slider disabled style={{ width: '100%'}} min={0} max={this.props.duration} value={this.props.currentSecond} onChange={this.handleRange} valueLabelDisplay="auto" valueLabelFormat={() => time} />
                {
                  this.props.marks.map((mark, index) => {
                    const timePosition = (mark.time*100)/this.props.duration;
